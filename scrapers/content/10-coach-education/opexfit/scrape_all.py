#!/usr/bin/env python3
"""
Scrape opexfit.com/blog via sitemap + HTML scraping.
...
Content selector: div.blog-item-content.e-content
AI crawlers blocked in robots.txt -- use browser User-Agent.
...
DOMAIN = "opexfit.com"
SITEMAP_URL = "https://www.opexfit.com/sitemap.xml"
SOURCE_TIER = "tier2"
SOURCE_CATEGORY = "10_coach_education"
# Browser UA -- robots.txt blocks ClaudeBot/anthropic-ai explicitly
USER_AGENT = (
    "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 "
    "(KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"
)
REQUEST_TIMEOUT = 30
DELAY = 1.5  # Slightly slower for Squarespace
PROGRESS_EVERY = 50
    ...
    try:
        ...
        for url_el in root.findall(".//sm:url/sm:loc", ns):
            if url_el.text:
                ...
                # Only blog posts (not the main /blog page itself)
                if "/blog/" in loc and loc.rstrip("/") != "https://www.opexfit.com/blog":
                    urls.append(loc)
...
def extract_article(html_bytes: bytes, url: str) -> dict | None:
    ...
    # Content: div.blog-item-content.e-content (Squarespace)
    content_el = soup.select_one("div.blog-item-content.e-content")
    if not content_el:
        # Fallback selectors for Squarespace
        content_el = soup.select_one(".sqs-block-content")
        if not content_el:
    ...
    if not date_published:
        ...
        if not date_published:
            # Squarespace often uses time.dt-published
            time_el = soup.select_one("time.dt-published")
            if time_el:
                date_published = time_el.get("datetime", "")
...
def main():
    ...
    print(f"OPEX Fitness Blog Scraper")
