#!/usr/bin/env python3
"""
Scrape blog.myfitnesspal.com via the Wayback Machine.

WordPress subdomain, Cloudflare blocked live. The probe found WP REST API is
preserved in Wayback snapshots. Strategy:
1. Try WP REST API via Wayback (paginate through ~6,994 posts)
2. If API fails, fall back to CDX URL discovery + per-page HTML scraping

Usage:
    python3 scrape_all.py                  # Auto: try API first, fall back to CDX
    python3 scrape_all.py --limit 10       # Scrape first 10 articles
    python3 scrape_all.py --cdx-only       # Skip API, use CDX discovery only
...
API_CACHE_DIR = os.path.join(SCRIPT_DIR, "api_cache")
...
SOURCE_DOMAIN = "blog.myfitnesspal.com"
SOURCE_TIER = "tier2"
...
# WP REST API via Wayback
WP_API_BASE = "https://blog.myfitnesspal.com/wp-json/wp/v2/posts"
# Use 2025 snapshot year for the API
WAYBACK_API_PREFIX = "https://web.archive.org/web/2025"
...
def extract_slug(url: str) -> str:
    """Extract slug from MFP blog URL."""
...
def try_wp_api_via_wayback() -> list[dict] | None:
    """
    Try to paginate through the WP REST API via Wayback Machine.
    Returns list of post dicts if successful, None if API is not available.
    """
    print("Trying WP REST API via Wayback Machine...")
    ...
    while page <= max_pages:
        cache_file = os.path.join(API_CACHE_DIR, f"page_{page}.json")
        ...
        if os.path.exists(cache_file):
            try:
                ...
                print(f"  Page {page}: {len(posts)} posts (cached)")
        ...
        # Fetch from Wayback
        api_url = f"{WP_API_BASE}?per_page={per_page}&page={page}"
        wayback_url = f"{WAYBACK_API_PREFIX}id_/{api_url}"

        time.sleep(REQUEST_DELAY)
        data = _make_request(wayback_url, timeout=60)

        if not data:
            if page == 1:
                print("  WP API not available via Wayback. Falling back to CDX.")
                return None
            else:
                print(f"  Page {page}: no data (may have reached end)")
                break

        try:
            text = data.decode("utf-8", errors="replace")
            # Wayback may wrap JSON in HTML -- try to extract JSON
            # If it starts with [ it's clean JSON
            if text.strip().startswith("["):
                ...
                if match:
                    ...
                    if page == 1:
                        print("  WP API response not valid JSON. Falling back to CDX.")
                        return None
                    break
    ...
    print(f"  Total from WP API: {len(all_posts)} posts")
    return all_posts if all_posts else None


def wp_post_to_article(post: dict, articles_dir: str, raw_dir: str) -> dict | None:
    """Convert a WP REST API post to a saved article."""
    ...
    # Extract content from WP API response
    title = ""
    ...
    if isinstance(title_data, dict):
        ...
        title = title_data
    title = html_unescape(BeautifulSoup(title, "lxml").get_text(strip=True)) if title else slug.replace("-", " ").title()

    # Content from rendered HTML
    ...
    # Convert to markdown
    body_md = md(content_html, heading_style="ATX", strip=["img"])
    body_md = re.sub(r"\n{3,}", "\n\n", body_md)
    ...
    # Extract metadata
    author = None
    # WP API has author as numeric ID -- we'd need another API call for name
    # Use Yoast data if available
    yoast = post.get("yoast_head_json", {})
    if yoast:
        author_data = yoast.get("author")
        if isinstance(author_data, str):
            author = author_data
        og = yoast.get("og_title", "")
        if not title and og:
            title = og

    date_published = post.get("date")
    date_modified = post.get("modified")
    link = post.get("link", f"https://blog.myfitnesspal.com/{slug}/")
...
# ---------------------------------------------------------------------------
# CDX Fallback - URL Discovery
# ---------------------------------------------------------------------------


def discover_urls_cdx() -> list[str]:
    """Use CDX API to discover blog.myfitnesspal.com article URLs."""
    all_urls = set()

    print("Discovering URLs via CDX API...")
    cdx_url = (
        "http://web.archive.org/cdx/search/cdx?"
        "url=blog.myfitnesspal.com/*&fl=original&collapse=urlkey"
...
def _is_article_url(url: str) -> bool:
    """Check if URL is a blog article (not WP system page)."""
    path = urllib.parse.urlparse(url).path.strip("/")
    if not path:
        return False
    ...
    skip_prefixes = (
        "wp-", "author/", "category/", "tag/", "page/",
        "feed", "xmlrpc", "comments", "trackback",
    ...
    if re.search(r"\.(jpg|jpeg|png|gif|css|js|xml|txt|ico|pdf)$", path, re.I):
        return False

    # Skip query parameters (pagination variants etc.)
    if "?" in url:
        return False

    # Must look like a slug (has hyphens or is a word)
    if "/" in path:
        # Might be category/slug or date/slug -- get the last part
        parts = [p for p in path.split("/") if p]
        if len(parts) > 3:  # Too many path segments
            return False
...
def extract_content(html: str, url: str) -> dict:
    """Extract content from MFP blog HTML page."""
        ...
        try:
            ...
            for item in items:
                if not isinstance(item, dict):
                    continue
                if "Article" in str(item.get("@type", "")):
                    json_ld = item
...
# ---------------------------------------------------------------------------
# CDX Fallback Scraper
# ---------------------------------------------------------------------------


def scrape_single_cdx(
    url: str,
    cdx_cache: dict,
    articles_dir: str,
    raw_dir: str,
    ...
    """Scrape a single URL via CDX+Wayback HTML."""
...
def main():
    parser = argparse.ArgumentParser(description="Scrape blog.myfitnesspal.com via Wayback Machine")
    parser.add_argument("--limit", type=int, default=0, help="Limit to first N articles (0 = all)")
    ...
    parser.add_argument("--cdx-only", action="store_true", help="Skip WP API, use CDX URL discovery")
    ...
    # Strategy 1: Try WP REST API via Wayback (unless --cdx-only)
    api_posts = None
    if not args.cdx_only and not args.scrape_only:
        api_posts = try_wp_api_via_wayback()

    if api_posts:
        print(f"\n=== Processing {len(api_posts)} posts from WP API ===")
        if args.limit > 0:
            api_posts = api_posts[:args.limit]
            print(f"Limit mode: processing first {len(api_posts)} posts")

        stats["total"] = len(api_posts)

        for i, post in enumerate(api_posts, 1):
            try:
                result = wp_post_to_article(post, ARTICLES_DIR, RAW_DIR)
            except Exception as e:
                slug = post.get("slug", "unknown")
                print(f"  [ERROR] Exception for {slug}: {e}")
                result = {"source_id": f"mfp-{slug}", "status": "error"}
            ...
            if i % 50 == 0 or i == len(api_posts):
                print(f"Progress: {i}/{len(api_posts)} "
                      f"(success={stats['success']}, skipped={stats['skipped']}, "
                      f"failed={stats['failed']})")

    else:
        # Strategy 2: CDX fallback
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            ...
            for url in urls:
                f = executor.submit(
                    scrape_single_cdx, url, cdx_cache, ARTICLES_DIR, RAW_DIR,
                    cdx_cache_lock,
                )
                futures[f] = url
    ...
    print(f"\n=== SCRAPE COMPLETE: blog.myfitnesspal.com ===")
