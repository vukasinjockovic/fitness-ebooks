#!/usr/bin/env python3
"""
Scrape biolayne.com content via WP REST API.

Fetches articles, research reviews, podcasts, and blog posts.
...
Caches author lookups to avoid redundant API calls.
...
BASE_URL = "https://biolayne.com/wp-json/wp/v2"
SOURCE_DOMAIN = "biolayne.com"
SOURCE_TIER = "tier1"
...
POST_TYPES = ["article", "rr_article", "podcast", "posts"]
...
# Author cache: author_id -> author_name
...
def build_user_api_url(user_id: int) -> str:
    """Build WP REST API URL for fetching a user by ID."""
...
def make_filename(slug: str, post_type: str, existing_filenames: set) -> str:
    """
    Generate filename for a post. Biolayne always uses prefix to avoid
    collisions between post types.
    """
...
def fetch_author_name(author_id: int) -> str:
    """
    Fetch author name from WP API. Caches results.
    Returns "Unknown" on failure.
    """
    if author_id in author_cache:
        return author_cache[author_id]

    url = build_user_api_url(author_id)
    try:
        req = urllib.request.Request(url)
        req.add_header("User-Agent", USER_AGENT)
        with urllib.request.urlopen(req, timeout=REQUEST_TIMEOUT) as resp:
        ...
        author_cache[author_id] = "Unknown"
        return "Unknown"
...
def scrape_post_type(post_type: str, existing_filenames: set) -> dict:
    ...
    page = 1
    first_url = build_api_url(post_type, page) + "&_embed"
    ...
    while page < total_pages:
        page += 1
        url = build_api_url(post_type, page) + "&_embed"
    ...
    for item in all_items:
        ...
        try:
            ...
            author_name = fetch_author_name(author_id) if author_id else "Unknown"
...
def main():
    ...
    print(f"BioLayne.com WP API Scraper")
