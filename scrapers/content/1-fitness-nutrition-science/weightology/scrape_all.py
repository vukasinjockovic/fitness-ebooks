#!/usr/bin/env python3
"""
Scrape free Weightology articles via WP REST API.

Fetches posts from weightology.net, filters out paywalled content,
converts HTML to markdown with frontmatter.

~380 total posts, ~75% paywalled. Expected yield: ~157 free articles.
...
# Configuration
# ---------------------------------------------------------------------------
DOMAIN = "weightology.net"
API_BASE = "https://weightology.net/wp-json/wp/v2/posts"
PER_PAGE = 100
USER_AGENT = "Mozilla/5.0 (compatible; GymZilla/1.0)"
DELAY = 1  # seconds between API pages
REQUEST_TIMEOUT = 30
DEFAULT_AUTHOR = "James Krieger"
SOURCE_TIER = "tier2"
...
# These match class names used by membership/restriction plugins
PAYWALL_DIV_PATTERN = re.compile(
    r'<div[^>]*class="[^"]*(?:restricted|rcp_restricted|members-only|member-content)[^"]*"[^>]*>',
    re.IGNORECASE
)
...
def is_paywalled(html_content: str) -> bool:
    """
    Detect if post content is behind a paywall.

    Checks for paywall marker divs (restricted-content, rcp_restricted, etc.)
    that contain subscribe/member language. Does NOT flag normal mentions of
    'subscribe' (e.g., newsletter signups) that appear outside paywall divs.
    """
    # Check for paywall div classes
    if PAYWALL_DIV_PATTERN.search(html_content):
        return True
    return False
...
def main():
    ...
    print(f"Weightology Scraper (free articles only)")
    print(f"Output: {ARTICLES_DIR}")
    print(f"\n--- Scraping Posts ---")
    print(f"API: {API_BASE}")

    stats = {"total": 0, "saved": 0, "skipped": 0, "paywalled": 0, "errors": 0}
    ...
    print(f"  Total: {total_items} posts across {total_pages} pages")
    stats["total"] = total_items

    # Process page 1
    for post in posts:
        try:
            if should_skip(post, ARTICLES_DIR):
                stats["skipped"] += 1
                continue
            if is_paywalled(post["content"]["rendered"]):
                stats["paywalled"] += 1
                continue
    ...
    for page in range(2, total_pages + 1):
        ...
        print(f"  Page {page}/{total_pages}: {len(posts)} posts")

    # Summary
    print(f"\n=== Summary ===")
    print(f"Total: {stats['total']} posts")
    print(f"Saved: {stats['saved']} free articles")
    print(f"Paywalled: {stats['paywalled']} (skipped)")
